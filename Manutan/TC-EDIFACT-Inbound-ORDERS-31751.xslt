  ...
  <xsl:template match="/">
	...
	<xsl:template match="UNH">
		...
		<PurchaseOrder>
			...
			<DTM>
				...
				<DocTime>
					<xsl:value-of select="substring(edi:getSubElement(DTM[Field[1]/Field[1] = '137'], 1, 2),9,6)"/>
				</DocTime>
				<PostingDate>
			...
			<RFF>
				...
				<BuyerOrderNum>
					<xsl:value-of select="edi:getSubElement(RFF[Field[1]/Field[1] = 'PO'], 1, 2)"/>
				</BuyerOrderNum>
				<VendorNum>
			...
			<ShipTo>
				...
				<CTA>
					<ContactName>
						<xsl:value-of select="edi:getSubElement($ShipTo/CTA[Field[1] = 'DL'], 2, 2)"/>
					</ContactName>
					<COM>
						<ContactPhone>
			...
			<Supplier>
				...
				<RFF>
					<VendorCode>
						<xsl:value-of select="edi:getSubElement($supplier/RFF[Field[1]/Field[1] = 'VR'], 1, 2)"/>
					</VendorCode>
				</RFF>
			</Supplier>
			<!--END NADs -->	
			...
			<PAT>
				...
				<TermsTimeRelation>
					<xsl:value-of select="edi:getSubElement(PAT, 3, 2)"/>
				</TermsTimeRelation>
				<TermsPeriodTypeCode>
			...
			<TDT>
				...
				<TransportMeansCode>
					<xsl:value-of select="edi:getSubElement(TDT, 3, 1)"/>
				</TransportMeansCode>
				<TransportMeansDescription>
					<xsl:value-of select="edi:getSubElement(TDT, 4, 2)"/>
				</TransportMeansDescription>
				<TransportCarrierID>
					<xsl:value-of select="edi:getSubElement(TDT, 5, 1)"/>
				</TransportCarrierID>
				<TransportDirection>
					<xsl:value-of select="edi:getSubElement(TDT, 6, 1)"/>
				</TransportDirection>
			</TDT>
			<TOD>
				<DeliveryTransportDesc>
					<xsl:value-of select="edi:getSubElement(TOD, 1, 1)"/>
				</DeliveryTransportDesc>
	...
	<xsl:template match="LIN">
		<Items>
			<LIN>
				...
				<VendorItemNum>
					...
					<IMD>
						...
						<DescriptionType>
							<xsl:value-of select="edi:getSubElement(IMD[Field[1] = 'F'], 3, 5)"/>
						</DescriptionType>
						<Variety2>
							<xsl:value-of select="edi:getSubElement(IMD[Field[1] = 'A'], 3, 4)"/>
						</Variety2>
						<PresentationQtyFormat>
							<xsl:value-of select="edi:getSubElement(IMD[Field[1] = 'A'], 3, 5)"/>
						</PresentationQtyFormat>
					</IMD>
					<DTM>
						<ItemDeliveryDate>
			...
			<PRI>
				<PriceQualifier>
					<xsl:value-of select="edi:getSubElement(PRI, 1, 1)"/>
				</PriceQualifier>
				<Price>
					<xsl:value-of select="edi:getSubElement(PRI, 1, 2)"/>
				</Price>
			</PRI>
			<LOC>
				<ItemPlaceofDelivery>
					<xsl:value-of select="edi:getSubElement(LOC[Field[1]/Field[1] = '7'], 1, 2)"/>
				</ItemPlaceofDelivery>
				<QTY>
					<LocationSplitQty>
			...
			<TAX>
				<VATTaxRate>
					<xsl:value-of select="edi:getSubElement(self::node()[Field[1]/Field[1] = '7'], 5, 4)"/>
				</VATTaxRate>
				<MOA>
					<TaxAmount>
					...
					<TaxCurrency>
						<xsl:value-of select="edi:getSubElement(TAX/MOA[Field[1]/Field[1] = '124'], 1, 3)"/>
					</TaxCurrency>
				</MOA>
			</TAX>
			<PCD>
