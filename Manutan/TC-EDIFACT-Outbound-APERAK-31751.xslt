...
<!--
	...
	Output: Cummins EANCOM D96A APERAK.
	
	Author: Charlie Tracy
	Version: 1.0
	Creation Date: 23-July-2017
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:date="com.css.base.xml.xslt.ext.XsltDateExtension" xmlns:str="com.css.base.xml.xslt.ext.XsltStringExtension" xmlns:edifact="com.css.base.xml.xslt.ext.edi.XsltParsedEdifactEdiExtension" xmlns:mapper="com.api.tx.MapperEngine" extension-element-prefixes="date mapper str edifact">
    ...
    <xsl:template match="Batch">
		...
		<xsl:variable name="receiverANA" select="/Batch/Aperak[1]/BatchReferences/ReceiverCode" />
        <!-- Some hubs specify different criterea in test and live modes -->
        <xsl:variable name="testMode" select="/Batch/Aperak[1]/BatchReferences/@test = 'true' or $TestMode = 'true'" />
        <xsl:variable name="vendorID">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="/Batch/Aperak[1]/Supplier/CustomersCode" />
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <Document type="EDIFACT" wrapped="false">
                ...
                <mapper:genNum>
                    <xsl:choose>
                        <xsl:when test="string-length($BatchRefText) &gt; 0">
                            <xsl:value-of select="concat(/Batch/Aperak[1]/BatchReferences/SenderCode, '.', $BatchRefText, '.', 'TESCO', '.', 'APERAK')" />
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="concat(/Batch/Aperak[1]/BatchReferences/SenderCode, '.', 'TESCO', '.', 'APERAK')" />
                        </xsl:otherwise>
                    </xsl:choose>
                </mapper:genNum>
            </xsl:variable>          
            ...
            <UNB>
                ...
                <Field>
                    <Field>
                        <xsl:value-of select="/Batch/Aperak[1]/BatchReferences/SenderCode" />
                    </Field>
                    <Field>
                        <xsl:value-of select="/Batch/Aperak[1]/BatchReferences/SenderCodeQualifier" />
                    </Field>
                </Field>
                <!-- Receiver ANA and Qualifier -->
                <Field>
                    <Field>
                        <xsl:value-of select="/Batch/Aperak[1]/BatchReferences/GSReceiverCode" />
                    </Field>
                    <Field>
                        <xsl:value-of select="/Batch/Aperak[1]/BatchReferences/ReceiverCodeQualifier" />
                    </Field>
                </Field>
                ...
                <Field>
                    <xsl:value-of select="/Batch/Aperak[1]/BatchReferences/BatchRef" />
                </Field>
                <!-- Network Password -->
                <Field>
                ...
                <Field>
                    <!-- Test Indicator -->
                        <xsl:value-of select="Batch/Aperak[1]/BatchReferences/test" />
                </Field>
                <!-- Process each ASN -->
                <xsl:apply-templates select="APERAK">
    ...
    <xsl:template match="APERAK">
        ...
        <UNH>
			...
			<xsl:if test="string-length(RFF/InvoiceNumber) &gt; 0">
			<RFF>
				<mapper:incVar name="segmentCount" />
				<Field>
					<Field>AGO</Field>
					<Field>
						<xsl:value-of select="RFF/InvoiceNumber" />
					</Field>
				</Field>
			</RFF>
			</xsl:if>
			<xsl:if test="string-length(RFF/SIDShippersID) &gt; 0">
			<RFF>
				<mapper:incVar name="segmentCount" />
				<Field>
					<Field>AOE</Field>
					<Field>
						<xsl:value-of select="RFF/SIDShippersID" />
					</Field>
				</Field>
			</RFF>
			</xsl:if>
			<NAD>
                ...
                <Field>
                    ...
                    <Field></Field>
                    <Field>
						<xsl:value-of select="NAD.MS/CodeType" />
					</Field>
				</Field>
				<Field></Field>
				<Field></Field>
			...
			<NAD>
                ...
                <Field>
                    <Field>
                        <xsl:value-of select="NAD.AK/Code" />
                    </Field>
                    <Field></Field>
                    <Field>
						<xsl:value-of select="NAD.AK/CodeType" />
					</Field>
				</Field>
            </NAD>
			<xsl:for-each select="Item">
				<ERC>
					...
					<Field>					
						<Field>
							<xsl:value-of select="ERC/Status" />
						</Field>
						<Field></Field>
						<Field>83</Field>
					</Field>
				...
				<mapper:incVar name="segmentCount" />				
						...
						<Field></Field>
						<Field>
							<xsl:value-of select="FTX/ErrorDescription" />
						</Field>
				</FTX>
			</xsl:for-each>
            <UNT>
