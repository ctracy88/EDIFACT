...
<xsl:stylesheet version="1.0" 
                ...
                extension-element-prefixes="date mapper str edifact">

    <xsl:output method="xml"/>

	<xsl:param name="SenderEnvelopeQualifier"/>
	<xsl:param name="TestMode"/> <!-- true if to override the partner default -->
	<xsl:param name="CustomerCodeForSupplier"/>	  <!-- Optional if not supplied in the generic XML -->
	<xsl:param name="Network"/> <!-- AS2 or TGMS. If not specified then it defaults to TGMS -->
	<xsl:param name="Container"/> <!-- If not in the Type/Customers or Type/Suppliers then you can use this. Set to CHEP or EURO -->
	<xsl:param name="BoxType"/> <!-- If not in the SuppliedIn then you can use this. Set to TRAY or BOX -->
    ...
    <xsl:template match="Batch">
			...
			<Document type="EDIFACT" wrapped="false">
				<xsl:attribute name="syntax"><xsl:value-of select="'UNOA'"/></xsl:attribute>
				<xsl:attribute name="version"><xsl:value-of select="'3'"/></xsl:attribute>
				...
				<mapper:logDetail>
					File Generation number: <xsl:value-of select="$BatchRef"/>
				</mapper:logDetail>
				...
				<UNB>
					...
					<Field>
						<Field><xsl:value-of select="/Batch/ASN[1]/BatchReferences/SenderCode"/></Field>
						<Field><xsl:value-of select="$SenderEnvelopeQualifier"/></Field>
					</Field>
					<!-- Receiver ANA and Qualifier -->
					<Field>
						<Field>
							<xsl:choose>
								<xsl:when test="$Network = 'AS2'">5000119000006</xsl:when>
								<xsl:when test="$testMode = 'true'">2513671255172</xsl:when>
								<xsl:otherwise>2513671255177</xsl:otherwise>
							</xsl:choose>						
						</Field>
						<Field>14</Field>
					</Field>
					...
					<Field>
						<Field><xsl:value-of select="date:insert('yyMMdd')"/></Field>
						<Field><xsl:value-of select="date:insert('hhmm')"/></Field>
					</Field>
					<!-- Interchange Reference Number -->
					<Field><xsl:value-of select="$BatchRef"/></Field>
					<!-- Network Password -->
					<Field><xsl:value-of select="$NetworkPassword"/></Field>
					<!-- Application Reference -->
					<Field>DESADV</Field>
					<Field/> <!-- Processing Priority -->
					<Field/> <!-- Acknowledgement Request -->
					<Field/> <!-- Communications Agreement -->
					<Field> <!-- Test Indicator -->
						<xsl:if test="/Batch/ASN[1]/BatchReferences/@test = 'true'"><xsl:value-of select="'1'"/></xsl:if>
					</Field>
		...
		<xsl:template match="ASN">
			...
			<UNH>
				...
				<Field>
					...
					<Field>96A</Field>
					<Field>UN</Field>
					<Field>DESAD3</Field> <!-- This is a back-office system Tesco hack -->
				</Field>

				<BGM>
					<mapper:incVar name="segmentCount"/>
					<Field>
						<Field>351</Field> <!-- Doc type 351 = despatch advice -->
						<Field/>
						<Field>9</Field> <!-- Message function 9 = original invoice -->
					</Field>
					<Field><xsl:value-of select="str:pad(DocumentNumber, 9, '0', 'true')"/></Field>
					<Field>9</Field> <!-- Message function 9 = original invoice -->
				</BGM>
				...
				<DTM>
					<mapper:incVar name="segmentCount"/>
					<Field>
						<Field>137</Field> <!-- qualifier, 137 = document date/time -->
						<Field>
							<xsl:value-of select="date:insert('yyyyMMddHHmm')"/>
						</Field>
						<Field>203</Field> <!-- date format, 203 = CCYYMMDDHHMM -->
					</Field>
				</DTM>
				...
				<DTM>
					...
					<Field>
						<Field>191</Field> <!-- qualifier, 191 = booking date/time -->
						<Field>
							<date:reformat curFormat="yyyy-MM-ddHH:mm" newFormat="yyyyMMddHHmm">
								<xsl:value-of select="concat(ExpectedDeliveryDate, '00:00')"/> <!-- 00:00 is OK if not supplied -->
							</date:reformat>
						</Field>
						<Field>203</Field> <!-- date format, 203 = CCYYMMDDHHMM -->
					</Field>
				...
				<xsl:if test="string-length(DeliverFrom/LoadingReferenceNumber) &gt; 0">
					<RFF>
						<mapper:incVar name="segmentCount"/>
						<Field>
							<Field>PK</Field> <!-- PK = packing list number -->
							<Field><xsl:value-of select="DeliverFrom/LoadingReferenceNumber"/></Field>
						</Field>
					</RFF>
				</xsl:if>
				...
				<NAD>
					<mapper:incVar name="segmentCount"/>
					<Field>BY</Field> <!-- BY = buyer -->
					<Field>
						<Field><xsl:value-of select="Customer/EanCode"/></Field>
						<Field/>
						<Field>9</Field> <!-- qualifier, 9 = EAN -->
					</Field>
				</NAD>

				<NAD>
					<mapper:incVar name="segmentCount"/>
					<Field>SE</Field> <!-- SE = seller -->
					<Field>
						<Field><xsl:value-of select="Supplier/EanCode"/></Field>
						<Field/>
						<Field>9</Field> <!-- qualifier, 9 = EAN -->
					</Field>
				</NAD>
				...
				<xsl:if test="Consignor/@outputNadCS != 'false'">
					<NAD>
						<mapper:incVar name="segmentCount"/>
						<Field>CS</Field> <!-- CS = consolidator -->
						<Field>
				...
				<TDT>
					<mapper:incVar name="segmentCount"/>
					<Field>20</Field> <!-- 20 = main carriage transport -->
					<Field/>
					<Field>30</Field> <!-- 30 = road transport -->
					<Field/>
					<Field>
						<Field><xsl:value-of select="Consignor/EanCode"/></Field>
						<Field/>
						<Field/>
						<Field><xsl:value-of select="Consignor/Name"/></Field>
					</Field>
					
					<LOC>
						<mapper:incVar name="segmentCount"/>
						<Field>7</Field> <!-- 7 = place of delivery -->
						<Field><xsl:value-of select="Consignee/EanCode"/></Field>
					</LOC>
				</TDT>

				<CPS>
					<mapper:incVar name="segmentCount"/>
					<Field>1</Field> <!-- 1 = top level, aka entire shipment -->
					
					<PAC>
						<mapper:incVar name="segmentCount"/>
						<Field><xsl:value-of select="count(Package)"/></Field> <!-- Number of packages in the consignment -->
					</PAC>
...
<!--							<Field>1</Field> -->							
							<Field><xsl:value-of select="'2'"/></Field> <!-- Tesco change from original spec (above) -->
							<Field><xsl:value-of select="position()"/></Field>

							<PAC>
								<mapper:incVar name="segmentCount"/>
								<Field>1</Field> <!-- Number of packages in the consignment, always 1 -->
								<Field>								
									<Field></Field> <!-- package code -->
									<!-- <Field>52</Field> --> <!-- 52 = Package barcoded UCC or EAN-128, only if supplied, or else leave blank -->
								</Field>
								<Field> <!-- 201 = Euro Pallet, 202 = CHEP pallet -->
									<xsl:choose>
								...
								<PCI>
									<mapper:incVar name="segmentCount"/>
									<Field>33E</Field> <!-- 33E = Serial Shipping Container Code (SSCC) -->

									<GIN>
										<mapper:incVar name="segmentCount"/>
										<Field>BJ</Field> <!-- BJ = Serial shipping container code -->
										...
										<Field tag="SSCC" minLen="18" maxLen="18">
											<xsl:choose>
												<xsl:when test="string-length(Markings/Barcode) = 18"><xsl:value-of select="Markings/Barcode"/></xsl:when>
												<xsl:when test="string-length(Markings/Barcode) = 0"><xsl:value-of select="'000000000000000000'"/></xsl:when>
												<xsl:otherwise>
													<mapper:logError>
														SSCC is not 18 digits: <xsl:value-of select="Markings/Barcode"/>
													</mapper:logError>
												</xsl:otherwise>
											</xsl:choose>											
										</Field>
									...
									<xsl:for-each select="Product">
										<LIN>
											<mapper:incVar name="segmentCount"/>
											<Field><xsl:value-of select="position()"/></Field> <!-- line number -->
											<Field/>
											<Field> <!-- EAN code of item -->
												<Field tag="LIN-Barcode" minLen="8" maxLen="14">
													<!-- May be Dun-14 or EAN-13, UPC-12 -->
													<xsl:choose>
														<xsl:when test="string-length(PackageCode) = 14"><xsl:value-of select="PackageCode"/></xsl:when>
														<xsl:when test="string-length(EanCode) = 14"><xsl:value-of select="EanCode"/></xsl:when>
														<xsl:when test="string-length(EanCode) = 13"><xsl:value-of select="EanCode"/></xsl:when>
...
-->														
														<xsl:otherwise>
															<mapper:logError>
																Barcode is not 13 or 14 digits: <xsl:value-of select="PackageCode"/>, <xsl:value-of select="EanCode"/>
															</mapper:logError>
														</xsl:otherwise>
													</xsl:choose>					
												</Field>
												<Field>EN</Field> <!-- EN = EAN code -->
											</Field>
											...
											<PIA>
												<mapper:incVar name="segmentCount"/>
												<Field>1</Field> <!-- 1 = additional information -->
												<Field>
													<Field><xsl:value-of select="$orderNumber"/></Field>
													<Field>ORD</Field> <!-- ORD = Order number -->
												</Field>
											</PIA>
											...
											<xsl:choose>
												<xsl:when test="string-length(DeliveryNoteNumber) &gt; 0">
													<PIA>
														...
														<Field>1</Field> <!-- 1 = additional information -->
														<Field>
															<Field><xsl:value-of select="DeliveryNoteNumber"/></Field>
															<Field>DEL</Field> <!-- DEL = delivery note number -->
														</Field>
													</PIA>
												</xsl:when>
												<xsl:when test="string-length(../../DeliveryNoteNumber) &gt; 0">
											...
											<QTY>
												<mapper:incVar name="segmentCount"/>
												<Field>
													<Field>12</Field> <!-- 12 = shipped quantity -->
													<Field><xsl:value-of select="edifact:convertToEdifactDecimal(Quantity/Amount)"/></Field>
												</Field>
											</QTY>
											...
											<xsl:if test="Quantity/LineWeight &gt; 0">
												<QTY>
													...
													<Field>
														<Field>12</Field> <!-- 12 = shipped quantity -->
														<Field><xsl:value-of select="edifact:convertToEdifactDecimal(Quantity/LineWeight)"/></Field>
														<Field>
															<xsl:choose>
																<xsl:when test="Quantity/MeasureIndicator = 'Kilogram'">KGM</xsl:when>
																<xsl:otherwise>
																	<mapper:logError>
																		Unrecognised UOM in weighted line: <xsl:value-of select="Quantity/MeasureIndicator"/>
																	</mapper:logError>
																</xsl:otherwise>
															</xsl:choose>
														</Field>
				...
				<CNT>
					<mapper:incVar name="segmentCount"/>
					<Field>
						<Field>2</Field> <!-- 2 = total number of lines -->
						<Field><xsl:value-of select="count(Package/Product)"/></Field>
					</Field>
				</CNT>
				
				<UNT>
					<mapper:incVar name="segmentCount"/>
					<Field><mapper:getVar name="segmentCount"/></Field> <!-- number of segments in message -->
					<Field><xsl:value-of select="$AsnUnhRef"/></Field> <!-- UNH reference number -->
				</UNT>
		...
		-->
		<xsl:template name="determine-product-case-type">
			<xsl:param name="name"/>
