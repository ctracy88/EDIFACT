				...
				<xsl:for-each select="QTY">
					...
					<Quantity>
						...
						<ScheduleDates>
							...
							<PeriodStartDate>
								<xsl:value-of select="edi:getSubElement(self::node()/DTM[Field[1]/Field[1] = '194'], 1, 2)"/>
							</PeriodStartDate>
							<PeriodStopDate>
								<xsl:value-of select="edi:getSubElement(self::node()/DTM[Field[1]/Field[1] = '206'], 1, 2)"/>
							</PeriodStopDate>
						</ScheduleDates>
						<RefNum>
							<ShipperNum>
