        ...
        <UNH>
			...
			<xsl:for-each select="Pack">
				...
				<xsl:for-each select="Item">
					...
					<xsl:if test="string-length(Details/HandlingSpecialInstructions) &gt; 0">
					<HAN>
						<mapper:incVar name="segmentCount" />
						<Field>
							<xsl:value-of select="Details/HandlingSpecialInstructions" />
						</Field>	
					</HAN>
					</xsl:if>
					<PCI>
					...
					<LIN>
						...
						<xsl:if test="string-length(LIN/PIA/BuyerItemNumber) &gt; 0">
						<PIA>
							<mapper:incVar name="segmentCount" />
							<Field>1</Field>
							<Field>
								<Field>
								<xsl:value-of select="LIN/PIA/BuyerItemNumber" />
								</Field>
								<Field>IN</Field>
								<Field></Field>
								<Field>91</Field>
