<?xml version="1.0"?>
<!--
	XSLT to transform an Edifact Purchase Order Change message into a Generix XML variation.
	
	Input: EDIFACT D93A/D96A/D97A ORDER CHANGE.
	Output: Generic XML Order Change.
		...
		<PurchaseOrderChange>
			...
			<BuyingParty>
				...
				<Country>
					...
					<CTA>
						<ContactName>
							<xsl:value-of select="edi:getSubElement($BuyingParty/CTA[Field[1] = 'PD'], 2, 2)"/>
						</ContactName>
					</CTA>
			</BuyingParty>
			<Broker>
			...
			<Supplier>
				...
				<Country>
					...
					<CTA>
						<ContactName>
							<xsl:value-of select="edi:getSubElement($supplier/CTA[Field[1] = 'SU'], 2, 2)"/>
						</ContactName>
					</CTA>
			</Supplier>
			<!--END NADs -->	
			...
			<PAT>
				...
				<TermsNetDaysDue>
					...
					<PCD>
						<DiscountPercent>
							<xsl:value-of select="edi:getSubElement(PAT/PCD[Field[1]/Field[1] = '12'], 1, 2)"/>
						</DiscountPercent>
					</PCD>
					<MOA>
					<TotalStoreCostFF>
			...
			<TOD>
				<DeliveryTransportDesc>
					<xsl:value-of select="edi:getSubElement(TOD, 2, 1)"/>
				</DeliveryTransportDesc>
	...
	<xsl:template match="LIN">
		<Items>
			<LIN>
				...
				<EANNum>
					<xsl:value-of select="edi:getSubElement(self::node()[Field[3]/Field[2] = ''], 3, 1)"/>
				</EANNum>
				<UPC>
				...
				<VendorItemNum>
					<xsl:value-of select="edi:getSubElement(PIA[Field[2]/Field[4] = '92'], 2, 1)"/>
				</VendorItemNum>
				<VariantName>
					<xsl:value-of select="edi:getSubElement(IMD[Field[3]/Field[1] = 'MPN'], 3, 4)"/>
				</VariantName>
				<VariantDescription>
					<xsl:value-of select="edi:getSubElement(IMD[Field[3]/Field[1] = 'ITM'], 3, 4)"/>
				</VariantDescription>
			</LIN>
			<PIA>
				<SuppliersArticleNum>
			...
			<PRI>
				<Price>
					<xsl:value-of select="edi:getSubElement(PRI[Field[1]/Field[1] = 'INV'], 1, 2)"/>
				</Price>
			</PRI>
			<PCD>
				<DutyPercent>
			...
			<PAC>
				<PackQty>
					<xsl:value-of select="edi:getElement(PAC, 2)"/>
				</PackQty>
				<PackageType>
					<xsl:value-of select="edi:getElement(PAC, 3)"/>
				</PackageType>
				<MEA>
					<Height>
					...
					<CartonQuantity>
						<xsl:value-of select="edi:getSubElement(PAC/MEA[Field[1] = 'CT'], 3, 2)"/>
					</CartonQuantity>
				</MEA>
			</PAC>
			<PCI>
				<CartonMarks>
					<xsl:value-of select="edi:getSubElement(PCI[Field[1] = 'ZZZ'], 2, 1)"/>
				</CartonMarks>
			</PCI>
			<LOC>
					<DeliveryLocation>
						<xsl:value-of select="edi:getSubElement(LOC[Field[1] = '7'], 2, 1)"/>
					</DeliveryLocation>
				<QTY>
					<SplitQuantity>
						<xsl:value-of select="edi:getSubElement(LOC/QTY[Field[1]/Field[1] = '11'], 1, 2)"/>
					</SplitQuantity>
					<TotalQtyPerLine>
						<xsl:value-of select="edi:getSubElement(LOC/QTY[Field[1]/Field[1] = '142'], 1, 2)"/>
					</TotalQtyPerLine>
				</QTY>
				<DTM>
					<EarlyDate>
						<xsl:value-of select="edi:getSubElement(LOC/DTM[Field[1]/Field[1] = '63'], 1, 2)"/>
					</EarlyDate>
					<LateDate>
						<xsl:value-of select="edi:getSubElement(LOC/DTM[Field[1]/Field[1] = '64'], 1, 2)"/>
					</LateDate>
				</DTM>
			</LOC>
			<NAD>
